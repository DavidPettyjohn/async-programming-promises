...
export async function get() {
    const { data } = await axios.get("http://localhost:3000/orders");// axios.get() returns a promise and is asynchronous therefore we can use await; now that await makes this sychonous, we can assign data parameter to the value of the get() call
